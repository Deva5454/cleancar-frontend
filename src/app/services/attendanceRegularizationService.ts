...
/**
 * Real resolution of the employee's city's City Manager - by designation +
 * city against the real employee database, not the disconnected
 * organizationHierarchyService (its seed cityManagerId values use a
 * different ID scheme than real employees and would never match any real
 * logged-in user).
 *
 ...
 */
function findCityManagerForCity(cityId: string): { id: string; name: string } | null {

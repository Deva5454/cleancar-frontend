...
// Reads real subscription + attendance data where available
const LS_SUBS = "cleancar_subscriptions";
const LS_CANCELLATIONS = "cleancar_cancellation_requests";

function readSubs(): any[] {
  try { return JSON.parse(localStorage.getItem(LS_SUBS) || "[]"); } catch { return []; }
}

function getRealRevenueMTD(): number {
  const now = new Date();
  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
  return readSubs()
    .filter(s => s.status === "Active" && new Date(s.startDate || s.createdAt) >= monthStart)
    .reduce((sum: number, s: any) => sum + (s.pricing?.finalPrice || s.pricing?.basePrice || 0), 0);
}

function getRealActiveSubscriptions(): number {
  return readSubs().filter(s => s.status === "Active").length;
}

function getRealCancelledMTD(): number {
  const now = new Date();
  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
  return readSubs().filter(s => {
    if (s.status !== "Cancelled") return false;
    return new Date(s.cancelledDate || s.startDate) >= monthStart;
  }).length;
}

function getRealChurnRate(): number {
  const active = getRealActiveSubscriptions();
  const cancelled = getRealCancelledMTD();
  return active > 0 ? parseFloat(((cancelled / (active + cancelled)) * 100).toFixed(1)) : 0;
}

...
/**
 * MyStockContent — the single, real implementation of "My Stock" for a
 * washer, shared by both the desktop route (MyStock.tsx) and the mobile
 * app's bottom-nav Stock tab (WasherMyStock.tsx). Previously the mobile
 * tab rendered a completely separate, fully hardcoded component with
 * non-functional buttons (STK-DEF-01) while this real implementation
 * only existed at a desktop-only route a field washer had no path to
 * (STK-DEF-06). There is now exactly one implementation.
 *
 * showBackButton is false in the mobile tab context, where a "Back"
 * button (which navigates browser history) doesn't make sense — tab
 * switching is already how you leave this screen there.
 */
export function MyStockContent({ showBackButton = true }: { showBackButton?: boolean }) {
                    ...
                    <div className="pt-3 border-t">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          // On a demo/fallback card, reorderLevel/balance
                          // are illustrative, not real — pre-filling a
                          // real request's quantity from them would seed
                          // it with a fabricated number (STK-DEF-07).
                          // Leave it for the washer to enter.
                          setRequestedQty(isDemoFallback ? "" : String(Math.max(1, stock.reorderLevel - stock.balance)));

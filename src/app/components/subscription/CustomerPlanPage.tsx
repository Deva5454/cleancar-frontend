...
export interface PackConfig { id: string; name: string; icon: string; price?: number; perLabel?: string; discount?: string; prices?: any; description?: string; validityDays?: number | null; perVisitLabel?: string; isUrgent?: boolean; }
...
export const DEFAULT_CONFIG: PlanPageConfig = {
  ...
  trustItems: ["ðŸ“¸ Before & after photos every wash","ðŸ  We come to you","ðŸ“ž Easy cancellation process"],
  trustStrip: ["ðŸ”’ Razorpay secured","ðŸ“¸ Before & after photos","ðŸ“ž Easy cancellation process","ðŸ  Home, office, society"],
  vehicleCategories: [
    { id: "hatchback", label: "Hatchback / Compact Sedan", icon: "ðŸš—" },
    { id: "suv", label: "SUV / Sedan / MUV", icon: "ðŸš™" },
    { id: "luxury", label: "Luxury / Large SUV", icon: "ðŸŽï¸" },
  ],
  carModelMap: {
  ...
  packs: [
    ...
    { id:"urgent",  name:"Urgent Wash", icon:"âš¡", description:"Shampoo+Wax Â· Washer arrives in 1 hour Â· Same day only Â· No reschedule Â· Amount forfeited if car unavailable", prices:{shampooWax:{hatchback:499,suv:599,luxury:799}}, discount:"Rs 100 premium", validityDays:0, isUrgent:true },
  ],
  commitments: [
  ...
  return (
    <div className="cpp-cost-panel" style={{position:"sticky",top:80,borderRadius:24,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
          ...
          <div style={{padding:"10px 12px",background:"linear-gradient(135deg,#f5f3ff,#ede9fe)",borderRadius:12,marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                <div style={{fontSize:13,fontWeight:700,color:"#1e293b"}}>{planMode==="monthly"?planObj?.icon:"ðŸ“¦"} {planDisplayName}</div>
      ...
      <div style={{background:"#f8fafc",padding:"12px 22px",borderTop:"1px solid #f1f5f9"}}>
        {["ðŸ”’ Razorpay secured payment","ðŸ“¸ Before & after photos"].map(t=>(
...
export function CustomerPlanPage() {
  ...
  const pincodeLabel = cfg.serviceablePincodes.find(p => p.code === pincode)?.label || pincode;
  ...
  const [bundleDiscountInfo,setBundleDiscountInfo]=useState<{discount:number;savings:number;totalPrice:number}|null>(null);
    ...
    if(step===5&&custMobile&&custMobile.length>=10&&!abandonLeadCaptured){
      try{
        addLead({
          ...
          notes:"Checkout abandon capture â€” customer reached payment step",
        });
  ...
  // G11: One-time wash max 10-day validity â€” customer cannot book more than 10 days ahead
  ...
  // SLIDE 8: Speed checkout â€” lookup returning customer by mobile number
  ...
  // â”€â”€â”€ GST VERIFICATION â€” Full API flow with demo mode â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // HOW IT WORKS:
  //   Step 1: Format check (instant, offline) â€” regex validates 15-char GSTIN structure
  //   Step 2: Live API call to GST portal via provider â€” returns legal name, status, state
  //
  // TO GO LIVE: change GST_DEMO_MODE to false and paste your API key
  //
  // API OPTIONS:
  //   A. gstincheck.co.in  â€” GET https://sheet.gstincheck.co.in/check/API_KEY/GSTIN
  //   B. Cashfree           â€” POST https://payout-gamma.cashfree.com/payout/v1/validation/gstin
  //   C. Eko (Bharat)       â€” GET https://staging.eko.in:25004/ekoapi/v1/tools/gst_number_info
  // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const GST_DEMO_MODE = true;         // â† false when API key is ready
  const GST_API_KEY = "YOUR_KEY";     // â† paste gstincheck.co.in key here

  const validateGST = async (gst: string) => {
    const clean = gst.toUpperCase().replace(/[^A-Z0-9]/g, "");
    setCustGST(clean);

    // Step 1 â€” Format validation (offline, instant)
    ...
    // Step 2 â€” Live API verification
    ...
    if (GST_DEMO_MODE) {
      // â”€â”€ DEMO MODE â”€â”€ simulates real API call with 1.2s delay â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ LIVE MODE (GST_DEMO_MODE = false) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    try {
      // Option A: gstincheck.co.in (recommended â€” simplest)
      ...
      // Network error â€” fall back to format-only validation
      setGstStatus("valid");
  ...
  const handleSubmit=async()=>{
    ...
    try {
      ...
      const sub=createSubscription({customerId,packageType:selectedPlan==="wax"?"ELITE_WASH":selectedPlan==="shampoo"?"SMART_WASH":"EXPRESS_WASH",packageName:planMode==="monthly"?(planObj?.name||selectedPlan||"Plan"):(packObj?.name||selectedPack||"Pack")+(bundleMonths>0?` Ã— ${bundleMonths} months`:""),frequency:isOneTime?"One-Time":selectedPack==="pack2"?"Pack of 2":selectedPack==="pack4"?"Pack of 4":"One-time",status:"Active",startDate:firstWashDate.toISOString().split("T")[0],renewalDate:renewalDate.toISOString().split("T")[0],pricing:{basePrice:effectiveBase||basePrice,discount:bundleMonths>0?bundleDiscountInfo?.savings||0:discountAmt,finalPrice:finalTotal,currency:"INR"},serviceDetails:{vehicleType:activeCat||"hatchback",addOns:addons,preferredTimeSlot:isOneTime?`${oneTimeDate} ${oneTimeHour}`:(selectedPack==="pack2"||selectedPack==="pack4")?(packStartOption==="immediate"?"Immediate â€” today":oneTimeDate&&oneTimeHour?`${oneTimeDate} ${oneTimeHour}`:prefTime):prefTime},billingCycle:"Monthly",paymentStatus:"Paid",bundleMonths:bundleMonths>0?bundleMonths:undefined,bundleId:undefined,isBundlePriority:bundleMonths>0||undefined,bundleDiscountPct:bundleMonths>0?bundleDiscountInfo?.discount:undefined,
      // C7: Store payment instrument for refund processing â€” pre-filled as Razorpay placeholder.
      // When Razorpay goes live, replace with actual instrument from payment response
      // (e.g. "HDFC Credit Card ending 4242" or "UPI â€” customer@upi").
      ...
      }).catch(()=>{/* non-blocking â€” localStorage is primary */});
      // 7ï¸âƒ£ Fire auto-job-creation events â€” JobContext listens for these
      try {
        if(planMode === "monthly") {
      ...
      // REVISED: Step 1 of 2-stage WA â€” pending message sent immediately after payment.
      ...
      if(notifyPref==="whatsapp"||notifyPref==="both"){
        ...
        // Team alert to TSM/Admin â€” new subscription notification (OM excluded per journey doc)
        try {
          const teamAlertMsg = `ðŸ”” *New Subscription â€” 24/9 Carwashing*\n\nCustomer: *${custName}*\nMobile: ${custMobile}\nVehicle: ${carModel} (${catLabel})\nPlan: *${invoice?.items?.[0]?.name||"Subscription"}*\nAmount: â‚¹${finalTotal.toLocaleString("en-IN")} (incl. GST)\nArea: ${pincodeLabel}\nInvoice: ${invNum}\n\nâ³ Action required: Assign washer within 2 working days.`;
          sendTeamAlert(cfg.brand.whatsappNumber, teamAlertMsg).catch(()=>{});
        } catch {}
        const gst2 = parseFloat((finalTotal*0.09).toFixed(2));
        const invoiceMsg = `ðŸ§¾ *Invoice â€” 24/9 Carwashing*\n\nInvoice: *${invNum}*\nDate: ${now.toLocaleDateString("en-IN")}\n${custGST&&gstStatus==="valid"?`ðŸ¢ *${custCompany||gstDetails?.legalName||""}*\nGSTIN: *${custGST}*\n`:""}\nService: *${invoice?.items?.[0]?.name||"Car Wash"}*\nAmount: â‚¹${finalTotal.toLocaleString("en-IN")}\nCGST 9%: â‚¹${gst2.toLocaleString("en-IN")}\nSGST 9%: â‚¹${gst2.toLocaleString("en-IN")}\n*Total Paid: â‚¹${(finalTotal+gst2*2).toLocaleString("en-IN")}*\n${custGST&&gstStatus==="valid"?"âœ… B2B GST Invoice â€” eligible for input tax credit\n":""}\nQueries: *080 48 79 45 45*`;
        sendWhatsApp(custMobile, invoiceMsg).catch(()=>{})
          .catch(()=>{/* non-blocking â€” wa.me fallback handled inside */});
      ...
      // G5 FIX: Do NOT fully convert referral at checkout â€” credit fires on first completed wash.
      ...
      // 8ï¸âƒ£ Create MultiMonthBundle record if customer bought a multi-month pack
      ...
      if (bundleMonths > 0 && (selectedPack === "pack2" || selectedPack === "pack4")) {
        try {
          ...
          console.error("[BuyPage] createBundle failed â€” subscription created but bundle record missing:", e);
        }
      }
  ...
  if (step===7) {
    ...
    const nextSteps: {icon:string;title:string;detail:string;color:string}[] =
      isMonthly ? [
        ...
        {icon:"ðŸ”„",title:"Auto-renewal",detail:`Renews ${commitLabel ? "after " + commitLabel.toLowerCase().replace("-","").trim() : "monthly"}. Cancel with 7 days' notice per policy.`,color:"#8b5cf6"},
      ] : isPack ? [
  ...
  return (
    <div ref={scrollRef} className="cpp-root" style={{minHeight:"100vh",background:"linear-gradient(160deg,#f0f4ff 0%,#faf5ff 35%,#f0fdff 70%,#fefce8 100%)"}}>
        ...
        <div className="cpp-step-in" key={step}>
          ...
          {step===3 && (
            <div style={{background:"rgba(255,255,255,0.8)",backdropFilter:"blur(16px)",borderRadius:24,padding:"32px",border:"1px solid rgba(255,255,255,0.8)",boxShadow:"0 8px 32px rgba(99,102,241,0.08)"}}>
              ...
              {/* â”€â”€ Multi-Month Bundle Selector (Pack of 2 / Pack of 4 only) â”€â”€ */}
              {planMode==="pack" && (selectedPack==="pack2"||selectedPack==="pack4") && (
                <div style={{marginTop:20,padding:"18px 20px",borderRadius:16,border:"2px solid rgba(99,102,241,0.25)",background:"linear-gradient(135deg,#f0f4ff,#faf5ff)"}}>
                  <div style={{fontSize:13,fontWeight:800,color:"#4338ca",marginBottom:4}}>ðŸ“… Want to buy multiple months upfront?</div>
                        ...
                        <div key={m} onClick={()=>{
                          ...
                          {m===0 ? (
                            ...
                            <>
                              ...
                              <div style={{fontSize:10,color:"#64748b"}}>â‚¹{discP}/mo</div>
                            </>
                          )}
                        </div>
                      );
                  ...
                  {bundleMonths>0 && bundleDiscountInfo && (
                    <div style={{padding:"10px 14px",borderRadius:10,background:"rgba(16,185,129,0.1)",border:"1.5px solid rgba(16,185,129,0.3)"}}>
                      <div style={{fontSize:12,fontWeight:700,color:"#059669"}}>
                        ðŸŽ‰ You save â‚¹{bundleDiscountInfo.savings.toLocaleString("en-IN")} by paying {bundleMonths} months upfront
                      </div>
                      <div style={{fontSize:11,color:"#064e3b",marginTop:4}}>
                        Total: â‚¹{bundleDiscountInfo.totalPrice.toLocaleString("en-IN")} Â· {(selectedPack==="pack2"?2:4)*bundleMonths} total visits Â· Priority 1-hour scheduling Â· Soft cap: {(selectedPack==="pack2"?2:4)*2} visits/month max
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div style={{background:"rgba(255,255,255,0.8)",backdropFilter:"blur(16px)",borderRadius:24,padding:"32px",border:"1px solid rgba(255,255,255,0.8)",boxShadow:"0 8px 32px rgba(99,102,241,0.08)"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
                {[
                  ...
                  {label:"Mobile number *",value:custMobile,onChange:setCustMobile,onBlur:()=>handleMobileLookup(custMobile),placeholder:"10-digit number",type:"tel",icon:"ðŸ“±"},
              ...
              {/* â”€â”€ GST / Business Invoice Section â”€â”€ */}
              <div style={{marginBottom:16,padding:16,borderRadius:14,border:"2px dashed rgba(99,102,241,0.25)",background:"rgba(248,250,252,0.9)"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                  ...
                  <span style={{fontSize:11,color:"#94a3b8",fontWeight:400}}>Optional â€” for businesses claiming input tax credit</span>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:10}}>
                  ...
                  <div>
                    <label style={{display:"block",fontSize:11,fontWeight:700,color:"#374151",marginBottom:6}}>
                      GSTIN
                      {gstStatus==="checking" && <span style={{color:"#f59e0b",marginLeft:6,fontWeight:400}}>â³ Verifying...</span>}
                      {gstStatus==="valid"    && <span style={{color:"#10b981",marginLeft:6,fontWeight:400}}>âœ… Verified</span>}
                      {gstStatus==="invalid"  && <span style={{color:"#ef4444",marginLeft:6,fontWeight:400}}>âŒ Not found</span>}
                    </label>
                    <input
                      className="cpp-input"
                      value={custGST}
                ...
                {gstStatus==="checking" && (
                  <div style={{padding:8,borderRadius:8,background:"#fffbeb",border:"1px solid #fde68a",fontSize:12,color:"#92400e"}}>
                    â³ Checking GSTIN with GST portal... (Demo mode active â€” simulates 1.2s API call)
                  </div>
                )}
                {gstStatus==="valid" && gstDetails && (
                  <div style={{padding:10,borderRadius:10,background:"#f0fdf4",border:"1px solid #bbf7d0",fontSize:12}}>
                    <div style={{fontWeight:700,marginBottom:4,color:"#065f46"}}>ðŸ¢ {gstDetails.legalName || gstDetails.tradeName || custCompany}</div>
                    ...
                    <div style={{marginTop:8,padding:6,background:"#dcfce7",borderRadius:6,color:"#059669",fontSize:11,fontWeight:600}}>
                      âœ“ B2B GST invoice generated Â· CGST 9% + SGST 9% Â· Input tax credit eligible
                    </div>
                    {GST_DEMO_MODE && <div style={{marginTop:4,fontSize:10,color:"#94a3b8"}}>âš ï¸ Demo mode â€” set GST_DEMO_MODE=false + add API key to go live</div>}
                  </div>
                )}
                {gstStatus==="invalid" && (
                  <div style={{padding:8,borderRadius:8,background:"#fef2f2",border:"1px solid #fecaca",fontSize:12,color:"#dc2626"}}>
                    âŒ GSTIN not found or inactive. Please check and re-enter. Format: 24AAAPL1234C1Z5 (15 characters)
                  </div>
                )}
              </div>
              ...
              {(()=>{
                ...
                if(isPack2or4) return (
                  <div style={{marginBottom:20,padding:"16px 18px",background:"linear-gradient(135deg,#f0fdf4,#dcfce7)",border:"2px solid #86efac",borderRadius:14}}>
                    <div style={{fontSize:13,fontWeight:700,color:"#15803d",marginBottom:12}}>ðŸ“¦ When do you want your first wash?</div>
                    <div style={{display:"flex",gap:10,marginBottom:10}}>{(["immediate","schedule"] as const).map(opt=>(
                      <button key={opt} onClick={()=>{setPackStartOption(opt);if(opt==="immediate"){setOneTimeDate(new Date().toISOString().slice(0,10));setOneTimeHour("");}}}
                        ...
                        {opt==="immediate"?"âš¡ Book Now (Today)":"ðŸ“… Schedule for Later"}
                      </button>
                    ))}</div>
                      ...
                      <div style={{marginTop:10}}>
                        ...
                        {getOneTimeSlots(new Date().toISOString().slice(0,10)).length===0&&(
                          <div style={{padding:"10px 12px",background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:8,fontSize:12,color:"#92400e"}}>
                            âš ï¸ No same-day slots available â€” please choose Schedule for Later.
                          </div>
                        )}
                      </div>
                    )}
                ...
                if(isOneTime) return (
                  <div style={{marginBottom:16}}>
                    {/* G10: Book Now (Today) option for one-time â€” mirrors pack2/pack4 flow */}
                    <div style={{display:"flex",gap:8,marginBottom:12}}>
                      {([{v:"today",label:"ðŸ“… Book Now (Today)",sub:"Immediate scheduling"},{v:"schedule",label:"ðŸ—“ï¸ Schedule Later",sub:"Pick a date up to 10 days ahead"}] as const).map(opt=>(
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.7)",backdropFilter:"blur(6px)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setShowTnC(null)}>
          <div className="cpp-modal-enter" style={{background:"white",borderRadius:24,padding:32,maxWidth:500,width:"100%",maxHeight:"75vh",overflow:"auto",boxShadow:"0 40px 80px rgba(0,0,0,0.3)"}} onClick={e=>e.stopPropagation()}>
            ...
            <p style={{color:"#64748b",fontSize:14,lineHeight:1.7}}>
              {showTnC==="terms"&&<>
                By subscribing to 24/9 Carwashing services, you agree to our service standards, usage policies, and payment terms. Services are available Monday to Saturday. Sunday is a company-wide rest day â€” no washes are scheduled on Sundays. Individual washer week-off days rotate across Monâ€“Sat; your service continues uninterrupted as the system automatically redistributes jobs to a cover washer on your washer's day off.
                <br/><br/>
                ...
                <br/><br/>
                <strong>Cancellation Channel:</strong> You may cancel your subscription via WhatsApp, email, or our online form at 249carwashing.genxa.in/cancel-service â€” a reference number is generated immediately upon submission.
                <br/><br/>
              ...
              {showTnC==="refund"&&<>
                ...
                <br/><br/>
                <strong>Multi-Month Pack Bundle Refunds:</strong> Refund eligibility is based on visits consumed, not time elapsed. If you have used fewer than 50% of your total bundle visits, you are eligible for a refund on unused visits less a 10% cancellation fee and up to 2% gateway charges. If 50% or more visits have been used, no refund is applicable â€” however you retain your remaining visits until the bundle end date. Advance use of visits (using more in one window) counts toward the 50% threshold.
              </>}
              {showTnC==="cancel"&&<>
                You may cancel your subscription with 7 days' written notice via WhatsApp, email, or our online cancellation form at <strong>249carwashing.genxa.in/cancel-service</strong> â€” a reference number is generated immediately. No cancellation fee applies to month-to-month plans. Lock-in plans (3, 6, 9, 12 months) may have different terms as specified at the time of purchase.
                <br/><br/>
                <strong>One-Time Wash:</strong> Full refund if cancelled more than 2 hours before the scheduled slot. No refund within 2 hours of the scheduled slot time. Note: after 3 reschedules the booking is locked and you must call us to make changes. Rescheduling and cancellation cutoffs are separate â€” a locked reschedule does not prevent cancellation.
                <br/><br/>
                <strong>Multi-Month Pack Bundle Cancellations:</strong> Cancellations are processed based on the percentage of visits consumed across your total bundle. Under 50% consumed â†’ partial refund (visits used charged at per-visit rate + 10% fee). Over 50% consumed â†’ no refund, but remaining visits stay active until your bundle end date. Priority scheduling (1-hour TAT) applies to all bundle visits throughout the validity period. Unused visits within an expired 30-day window are forfeited â€” they cannot be used in future windows.
              </>}
            </p>
      ...
      {/* â“˜ Plan Info Modal â€” triggered from plan/pack/addon â“˜ buttons */}
      {infoModal && (
        <PlanInfoModal

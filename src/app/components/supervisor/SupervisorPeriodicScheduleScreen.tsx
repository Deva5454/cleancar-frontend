...
import {
  ...
  CheckCircle2, Clock, User, Repeat2, Lock, Droplets, Sparkles, Wind, Circle, LayoutPanelTop, Settings, Flower2, Droplet, PanelTop,
} from "lucide-react";
...
const SERVICE_ICON_MAP: Record<string, React.ReactNode> = {
  shampoo:   <Droplets className="w-4 h-4 text-blue-500" />,
  wax:       <Sparkles className="w-4 h-4 text-amber-500" />,
  interior:  <Wind className="w-4 h-4 text-teal-500" />,
  tyre:      <Circle className="w-4 h-4 text-gray-700" />,
  dashboard: <LayoutPanelTop className="w-4 h-4 text-indigo-500" />,
  engine:    <Settings className="w-4 h-4 text-gray-600" />,
  fragrance: <Flower2 className="w-4 h-4 text-pink-500" />,
  underbody: <Droplet className="w-4 h-4 text-cyan-600" />,
  glass:     <PanelTop className="w-4 h-4 text-sky-500" />,
};
                  ...
                  {(Object.entries(row.monthlyUsage) as [string, { used: number; cap: number }][])
                    ...
                    .map(([svc, v]) => (
                      <span key={svc} className={`text-xs px-1.5 py-0.5 rounded border ${
                        ...
                        {SERVICE_ICON_MAP[svc]}
                        {" "}{usageBar(v.used, v.cap)}
                        {v.used >= v.cap && <Lock className="inline w-3 h-3 ml-0.5" />}
                      </span>
                    ))}
                </div>
              </div>
                ...
                return (
                  <div key={occ.id}
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-xl">{SERVICE_ICON_MAP[occ.serviceType]}</span>
                      <div>
        ...
        <div style={{position:"fixed",inset:0,zIndex:10001,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
          <div style={{background:"white",borderRadius:"16px",padding:"24px",width:"100%",maxWidth:"400px"}}>
            ...
            <p style={{fontSize:"13px",color:"#475569",marginBottom:"4px"}}>
              {SERVICE_ICON_MAP[rescheduleTarget.occ.serviceType]}{" "}

...
// Emergency console unlock for Super Admin
if (typeof window !== 'undefined') {
  (window as any).unlockAdmin = () => {
    try {
      const key = 'EMPLOYEE_DATABASE_RECORDS';
      const emps = JSON.parse(localStorage.getItem(key) || '[]');
      const i = emps.findIndex((e: any) => e.loginMobile === '9100000001' || e.role === 'Super Admin');
      if (i >= 0) {
        emps[i].accountStatus = 'active';
        emps[i].failedLoginAttempts = 0;
        delete emps[i].lockedUntil;
        localStorage.setItem(key, JSON.stringify(emps));
        console.log('✅ Super Admin unlocked. Refresh the page.');
      } else {
        console.log('Not found. Try: localStorage.clear(); location.reload()');
      }

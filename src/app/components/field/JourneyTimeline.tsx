 ...
 * JourneyTimeline.tsx
 *
 * Real, shared PagarBook-style journey view - extracted so both the
 * live dashboard (LiveLocationDashboard.tsx) and the historical/admin
 * view (FieldAttendanceAdmin.tsx) show the exact same real, detailed
 * check-in -> drive -> stop -> checkout timeline with timestamped GPS
 * pins, rather than the admin view only showing a bare route line with
 * no stop detail or timestamps at all.
 */

import { useMemo } from "react";
import {
  MapPin, Clock, Footprints, CheckCircle2, XCircle,
  IndianRupee, Coffee, Car,
} from "lucide-react";
import { type GeoPoint, type FieldSession } from "../../services/fieldTrackingService";
...
export const staleMin = (iso: string) => Math.round((Date.now() - new Date(iso).getTime()) / 60000);
...
export function buildJourney(session: FieldSession): JourneyEvent[] {
...
export function JourneyPanel({ session }: { session: FieldSession }) {

...
export function CMAlertSystem({ alerts, onDismiss, onTakeAction }: CMAlertSystemProps) {
  ...
  const getAlertIcon = (type: CMAlert["type"]) => {
    const iconMap = {
      ...
      // V8 alert types — previously missing from this map entirely, which
      // meant a real CRM_DISCIPLINE_GAP alert (confirmed generated by
      // clusterManagerService.ts) crashed this whole screen, since
      // rendering `undefined` as a component throws React error #130.
      EARLY_CHURN_7DAY: TrendingDown,
      EARLY_CHURN_30DAY: TrendingDown,
      LOW_RENEWAL_RATE: XOctagon,
      CRM_DISCIPLINE_GAP: Users,
      EBITDA_RISK_PATTERN: Activity,
      SALES_QUALITY_ISSUE: Users,
      SLA_BREACH_SPIKE: Clock,
      RENEWAL_RATE_CRITICAL: XOctagon,
      RENEWAL_DECLINE_TREND: TrendingDown,
    };
    // Fallback for any type not in the map — old or new — so a future
    // alert type added to the CMAlert union can never crash this screen
    // again just because this map wasn't updated at the same time.

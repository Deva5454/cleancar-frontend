...
export interface Customer {
  ...
  // Multi-vehicle support: additional vehicles beyond the primary
  vehicles?: Array<{
    vehicleId: string;          // unique ID e.g. VEH-CUST-001-1
    category: string;           // Hatchback, Sedan, SUV, Luxury, 2W
    brand: string;
    model?: string;
    color: string;
    registrationNumber: string;
    isPrimary: boolean;
    addedAt: string;
    activeSubscriptionId?: string; // subscription linked to this vehicle
  }>;
  leadSource?: string; // "Referral", "Google Ads", "Walk-in", etc.
  referredBy?: string;          // referral code used at signup
  referralCode?: string;        // this customer's own referral code (generated after first purchase)
  referralRewardBalance?: number; // ₹ reward balance from referrals

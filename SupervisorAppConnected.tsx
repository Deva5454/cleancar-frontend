...
export function SupervisorAppConnected() {
  ...
  const handleReportPreDamage = () => {
    setShowPreDamageModal(true);
  };
  const handleSubmitPreDamage = () => {
    if (auditFlow) setAuditFlow({ ...auditFlow, preDamageReported: true });
    setShowPreDamageModal(false);
    setPreDamageNote("");
    toast.warning("Pre-damage logged and saved.", { duration: 3000 });
  };
  ...
  const handleSubmitAudit = () => {
    ...
    const action = fieldAuditService.getResultAction(result, auditFlow.preDamageReported || false);
    // Save record
    const record = { id: `AUDIT-${Date.now()}`, washerId: auditFlow.washerId, washerName: auditFlow.washerName, completedAt: new Date().toISOString(), score, result, photoCount: auditFlow.photos, preDamageReported: auditFlow.preDamageReported || false };
    const all = JSON.parse(localStorage.getItem("AUDIT_RECORDS") || "[]");
    localStorage.setItem("AUDIT_RECORDS", JSON.stringify([...all, record]));
    setAuditResult({ washerName: auditFlow.washerName, score, ...action });
    setAuditFlow(null);
  };
      ...
      {showPreDamageModal && (
        <div style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
          <div style={{background:"white",borderRadius:"16px",padding:"24px",width:"100%",maxWidth:"400px"}}>
            <h3 style={{fontWeight:700,fontSize:"18px",marginBottom:"8px",color:"#dc2626"}}>⚠️ Report Pre-Existing Damage</h3>
            <p style={{fontSize:"13px",color:"#6b7280",marginBottom:"12px"}}>Document damage found before the wash. This protects washer and company.</p>
            <textarea style={{width:"100%",border:"1px solid #d1d5db",borderRadius:"8px",padding:"8px 12px",fontSize:"14px",minHeight:"80px",marginBottom:"12px",resize:"none"}} placeholder="Describe the damage..." value={preDamageNote} onChange={e=>setPreDamageNote(e.target.value)} />
            <div style={{display:"flex",gap:"8px"}}>
              <button onClick={()=>setShowPreDamageModal(false)} style={{flex:1,padding:"10px",border:"1px solid #d1d5db",borderRadius:"8px",background:"white",cursor:"pointer",fontWeight:600}}>Cancel</button>

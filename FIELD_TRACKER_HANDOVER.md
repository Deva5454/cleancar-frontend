...
- **Resolve §7.1 (the two employee datasets) — still open, needs a decision.** This is the highest-leverage fix in the whole module; almost everything else found in this document was a symptom of it. See §7.1 for why it wasn't attempted here (cross-module blast radius, needs its own scoped test pass).
- **Add a dedicated `"Travel"` `Payable.type` — flagged, not done, needs a decision.** Currently travel claims are typed generically `"Salary"`, distinguished only by the presence of `travelTripId`. Splitting them into their own type would let Finance reporting filter travel reimbursements cleanly — but `"Salary"` payables currently feed **labour-cost calculations in `AnalyticsDashboardWithDrillDown.tsx`, `CityComparison.tsx`, `UnitEconomicsDashboard.tsx`, and `payrollFinanceService.ts`**. Splitting the type would silently shrink "labour cost" on those dashboards unless each one is updated to also include `"Travel"` — a cross-module Finance/Analytics change outside this pass's tested scope, same risk profile as §7.1. Needs a product decision (should travel reimbursement count as labour cost in those dashboards or not?) before touching it.
